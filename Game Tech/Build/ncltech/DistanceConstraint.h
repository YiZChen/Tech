...
public:
	...
	virtual void ApplyImpulse() override
	{
		...
		Vector3 v1 = m_pObj2->GetLinearVelocity() + Vector3::Cross(m_pObj2->GetAngularVelocity(), r2);
		{
			float  constraintMass =
				...
				+ Vector3::Dot(abn, Vector3::Cross(m_pObj1->GetInverseInertia()* Vector3::Cross(r1,
			...
			if (ab.Length() > abn.Length() * springMaxLength)
			{
				m_pObj2->SetPosition(m_pObj1->GetPosition() + abn * springMaxLength);
			}

			float  distance_offset = ab.Length() - m_Distance;
			float  baumgarte_scalar = 0.1f;
			float  b = -(baumgarte_scalar / PhysicsEngine::Instance()->GetDeltaTime()) * distance_offset;
	...
	virtual void DebugDraw() const
	{
		...
		NCLDebug::DrawThickLineNDT(globalOnA, globalOnB, 0.02f, Vector4(1.0f, 0.0f, 1.0f, 1.0f));
