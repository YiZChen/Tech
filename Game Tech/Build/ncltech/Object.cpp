...
void Object::RemoveGameObject(Object* child)
{
	auto loc = std::find(m_vpChildren.begin(), m_vpChildren.end(), child);

	if (loc != m_vpChildren.end())
	{
		m_vpChildren.erase(loc);
	}
	else
	{
		for (Object* c : m_vpChildren)
		{
			c->RemoveGameObject(child);
		}
	}
}