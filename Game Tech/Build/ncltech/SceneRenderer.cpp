...
: OGLRenderer()
	...
	, m_InvLightDirection(0.0f, 1.0f, 0.0f)
	, m_SpecularIntensity(20.0f)
...
bool SceneRenderer::InitialiseGL()
{
	...
	//Yiz's SKybox
	m_SkyBoxCubeTex = SOIL_load_OGL_cubemap(
		TEXTUREDIR"Space_lf.png", TEXTUREDIR"Space_rt.png",
		TEXTUREDIR"Space_up.png", TEXTUREDIR"Space_dn.png",
		TEXTUREDIR"Space_ft.png", TEXTUREDIR"Space_bk.png",
		SOIL_LOAD_RGB, SOIL_CREATE_NEW_ID, 0);
	if (!m_SkyBoxCubeTex)
	{
		NCLERROR("Unable to load Skybox Texture!");
		return false;
	}

	m_pQuad = Mesh::GenerateQuad();
...
void SceneRenderer::RenderScene()
{
	...
	//Main Render Window
	{
		...
		SetCurrentShader(m_pSkyBoxShader);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_CUBE_MAP, m_SkyBoxCubeTex);
		m_pQuad->Draw();
		glCullFace(GL_BACK);
		glEnable(GL_DEPTH_TEST);
...
bool SceneRenderer::ShadersLoad()
{
	...
	m_pSkyBoxShader = new Shader(
		SHADERDIR"SceneRenderer/SkyboxVertex.glsl",
		SHADERDIR"SceneRenderer/SkyboxFragment.glsl");
...
void SceneRenderer::ShadersSetDefaults()
{
	...
	glUniform1i(glGetUniformLocation(currentShader->GetProgram(), "uBumpTex"), 1);
	...
	SetCurrentShader(m_pSkyBoxShader);
	glUniformMatrix4fv(glGetUniformLocation(currentShader->GetProgram(), "uProjMtx"), 1, false, (float*)&projMatrix);
	glUniformMatrix4fv(glGetUniformLocation(currentShader->GetProgram(), "uViewMtx"), 1, false, (float*)&viewMatrix);
