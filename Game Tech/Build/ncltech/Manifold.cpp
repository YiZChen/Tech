...
void Manifold::SolveContactPoint(ContactPoint& c)
{
	...
	// Collision  Resolution
	{
		float  constraintMass = (m_pNodeA->GetInverseMass()
			...
			+ Vector3::Dot(normal, Vector3::Cross(m_pNodeA->GetInverseInertia()
	...
	//  Friction
	{
		...
		if (tangent_len > 0.001f)
		{
			...
			float  frictionalMass =
				(m_pNodeA->GetInverseMass() + m_pNodeB->GetInverseMass())
				+ Vector3::Dot(tangent, Vector3::Cross(m_pNodeA->GetInverseInertia()
			...
			float  oldImpulseTangent = c.sumImpulseFriction; //(<-- This should be a float not a Vector3
