...
public:
	...
	virtual void OnInitializeScene() override
	{
		...
		Object* bullet = CommonUtils::BuildSphereObject("bullet",
			Vector3(1.0f, 1.0f, 1.0f),				//Position leading to 0.1 meter overlap on faces, and more on diagonals
			0.5f,									//Radius
			true,									//Has Physics Object
			0.0f,									//Infinite Mass
			true,									//Has Collision Shape
	...
	virtual void OnUpdateScene(float dt) override
	{
		...
		if (m_Rotating)
		{
			...
			//Collision Colour (when collision happened)
			Vector4 dectectedCol = CommonUtils::GenColour(0.7f, 0.5f);
			...
			orbiting_sphere1->SetColour(orbiting_sphere1->Physics()->IsColliding() ? dectectedCol : beforeCol);
			rotating_cuboid1->SetColour(rotating_cuboid1->Physics()->IsColliding() ? dectectedCol : beforeCol);
			orbiting_sphere2->SetColour(orbiting_sphere2->Physics()->IsColliding() ? dectectedCol : beforeCol);
			

			Object* b = this->FindGameObject("bullet");
			b->Physics()->SetPosition(Vector3(0, 0, 0));
			b->Physics()->SetLinearVelocity(Vector3(0, 1, 0));
		}
